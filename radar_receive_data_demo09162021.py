...
class PacketGroup:
    ...
    def far_is_ready(self):
        ...
        self.curFarBufId += 1
        # print(f"far_is_ready,self.curFarBufId :{self.curFarBufId}")

    def near_is_ready(self):
        self.NearPackets.append([])
        self.curNearBufId += 1
        # print(f"near_is_ready,self.curNearBufId :{self.curNearBufId}")

    def far_del(self):
        del self.FarPackets[0]
        self.curFarBufId -= 1
        # print(f"far_del,self.curFarBufId :{self.curFarBufId}")

    def near_del(self):
        del self.NearPackets[0]
        self.curNearBufId -= 1
        # print(f"near_del,self.curNearBufId :{self.curNearBufId}")
    
    def output(self):
        pc_data=[]
        for i in range(len(self.FarPackets[0])):
            for j in range(len(self.FarPackets[0][i].Detections)):
                tmp = self.FarPackets[0][i].Detections[j][0]# posX
                pc_data.extend(float2bytesarray(tmp))
                tmp = self.FarPackets[0][i].Detections[j][1]# posY
                pc_data.extend(float2bytesarray(tmp))
                tmp = self.FarPackets[0][i].Detections[j][2]# posZ
                pc_data.extend(float2bytesarray(tmp))
                tmp = self.FarPackets[0][i].Detections[j][6]# RCS
                pc_data.extend(float2bytesarray(tmp))
        for i in range(len(self.NearPackets[0])):
            for j in range(len(self.NearPackets[0][i].Detections)):
                tmp = self.NearPackets[0][i].Detections[j][0]# posX
                pc_data.extend(float2bytesarray(tmp))
                tmp = self.NearPackets[0][i].Detections[j][1]# posY
                pc_data.extend(float2bytesarray(tmp))
                tmp = self.NearPackets[0][i].Detections[j][2]# posZ
                pc_data.extend(float2bytesarray(tmp))
                tmp = self.NearPackets[0][i].Detections[j][6]# RCS
                pc_data.extend(float2bytesarray(tmp))
        self.far_del()
        self.near_del()
        return pc_data
